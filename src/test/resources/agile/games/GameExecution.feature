...
Feature: Game Execution of Tap the Shoulder
  ...
  Scenario: As the facilitator starts the assignment, players gets two other players as goal
    Given a player at position 1,1 named "Jill"
  ...
  Scenario: As a player has done the first goal and is next to their second goal, that goal is
  accomplished.
    Given a game is in phase "assignment"
  ...
  Scenario: When a player has done their two goals and returned to the starting position, they are
  done.
    Given a game is in phase "assignment"
    And a player at position 3,2 named "P"
    And another player named "A"
